...
// MasterChef is the master of dogecorn. He can make dogecorn and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once dogecorn is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of dogecorn
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. dogecorn to distribute per block.
        uint256 lastRewardBlock;  // Last block number that dogecorn distribution occurs.
        uint256 accMilkPerShare;   // Accumulated dogecorn per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The dogecorn TOKEN!
    DogeCorn public milk;
    // Dev address.
    address public devaddr;
    // dogecorn  created per block.
    ...
    function updatePool(uint256 _pid) public {
        ...
        /*
        Example to calculate dogecorn Reward

        With 1 dogecorn per block
        ...
        1 * 3(multiplier) * 2500 = (7500) / 10000(totalAllocPoint) = 0.75 dogecorn tokens to be minted
        */
        uint256 milkReward = multiplier.mul(MilkPerBlock).mul(pool.allocPoint).div(totalAllocPoint); 
    ...
    // Deposit LP tokens to MasterChef for dogecorn allocation.
    ...
    // Safe dogecorn transfer function, just in case if rounding error causes pool to not have enough dogecorn.
